...
--------------------------------------------------------------------------------
function die()
  local size = config.getParameter("creeperSize")

  if size == "medium" then
    local entityId = world.spawnMonster("microcreeper", monster.toAbsolutePosition({ -1, 4 }), { level = monster.level() })
    world.callScriptedEntity(entityId, "setSpawnDirection", -1)

    entityId = world.spawnMonster("microcreeper", monster.toAbsolutePosition({ 1, 4 }), { level = monster.level() })
    world.callScriptedEntity(entityId, "setSpawnDirection", 1)
  end
end
...
--------------------------------------------------------------------------------
function creeperSize()
  return config.getParameter("creeperSize")
end
